...
export class StockEstimatorComponent implements OnInit {
  ...
  fetchStockItems(duration:number, dateduration: string){
    ...
    this.itemService.postGetEstimatedItems(duration,dateduration,localStorage.getItem('email'),localStorage.getItem('password')).subscribe(
      result => {
        ...
        $('.estimatestock-select').empty().select2({data: result['estimatedstock'],
        "language": {
          "noResults": function() {
            return resultdata;
          }
