...
async function onRequest(item) {
  if (item.utilizationTime) delete item.utilizationTime;
  if (item.utilizationQuantity) delete item.utilizationQuantity;
  if (item.utilizationUnits) delete item.utilizationUnits;
...
async function onUpdateRemoveRequest(newstock, newquantity, newunits, item) {
  if (newstock) {
    if (item.units == "kg" || item.units == "lit") {
      item.totalstock.amount -= newstock * item.quantity * 1000;
      if (item.totalstock.amount <= 0) item.totalstock.amount = 0;
      item.stockcount = Math.ceil(
        item.totalstock.amount / (item.quantity * 1000)
      );
    } else {
...
async function onUpdateAddAutomatic(newstock, newquantity, newunits, item) {
  if (newstock) {
    ...
    else newamount = newstock * item.quantity;
...
async function onUpdateRemoveAutomatic(newstock, newquantity, newunits, item) {
  if (newstock) {
    ...
    if (item.totalstock.amount <= 0) {
      ...
      item.totalstock.daysleft = Math.floor(
...
async function getNewStockCount(item, futuredate) {
  ...
  let daysrequired = moment(futuredate).diff(
...
module.exports = {
  onRequest,
  onUpdateAddRequest,
  onUpdateRemoveRequest,
  onAutomatic,
  onUpdateAddAutomatic,
  onUpdateRemoveAutomatic,
  getNewStockCount,
};
