...
export class CompleteStockEstimatorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    $('.estimatestock-select').on('select2:unselecting', (e) => {
      ...
      let removeditem = this.completeresult.find((row) => row.text == removedtext)
      ...
      $('.estimatestock-table').DataTable({...this.dtoptions,data: this.tabledata.map((obj) => [obj.category,obj.text,obj.nextreqdate,obj.currentstockcount,obj.newstockcount,obj.price])});
    ...
    $('.estimatestock-select').on('select2:selecting', (e) => {
      ...
      $('.estimatestock-table').DataTable().destroy();
      $('.estimatestock-table').DataTable({...this.dtoptions,data: this.tabledata.map((obj) => Object.values(obj))});
  ...
  fetchCompleteStockItems(duration:number, dateduration: string){
    this.isLoading$ = true;
    this.itemService.postGetCompleteEstimatedItems(duration,dateduration,localStorage.getItem('email'),localStorage.getItem('password')).subscribe(
      result => {
        ...
        for(let i=0;i<result['estimatedstock'].length;i++){
          ...
          this.completeresult[i].category = result['estimatedstock'][i].category;
          this.completeresult[i].name = result['estimatedstock'][i].name;
          this.completeresult[i].nextreqdate = result['estimatedstock'][i].nextreqdate;
          this.completeresult[i].currentstockcount = result['estimatedstock'][i].currentstockcount;
          this.completeresult[i].newstockcount = result['estimatedstock'][i].newstockcount;
        ...
        $('.estimatestock-select').empty().select2({data: selectArr,
        "language": {
          "noResults": function() {
            return resultdata;
          }
        ...
        for(let i=0;i<result['estimatedstock'].length;i++){
          ...
          if(!this.tabledata[i].price){
            ...
            this.totalprice =this.totalprice + parseInt(this.tabledata[i].price);
          }
        }
